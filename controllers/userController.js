...
const allocateMultipleIncharge = asyncHandler(async(req,res) => {
  let {incharge, interviewer} = req.body;

  await Incharge.updateOne({ _id: incharge }, { $push: { interviewer: interviewer } });

  req.flash('success', 'Incharge Allocated Successfully');
  res.redirect('/users');
});


const deallocateIncharge = asyncHandler(async (req, res, next) => {
  
  req.params = params(req);

  let {incharge, interviewer} = req.params;
  
  const data = await Incharge.find({_id: incharge});

  if(data.length > 0 && data[0].interviewer.length == 1 ){
    const user = await Incharge.findById(incharge);
    user.remove();
    res.redirect('/users');
    req.flash('success', 'User Successfully Deleted');
  }else{
    await Incharge.updateOne({_id: incharge}, {$pull: {interviewer: interviewer} }); 
    res.redirect('/users'); 
    req.flash('success', 'User Successfully Deleted');
  };
...
// const studentFeedback = asyncHandler(async(req,res) => {
//   const {feedback} = req.body;
  
//   try {

//     await StudentFeedback.create(feedback);
